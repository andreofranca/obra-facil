...
import { Settings, Search, Filter, Plus, ChevronRight, MoreVertical } from 'lucide-react';
      ...
      <main className="px-6 space-y-8 w-full max-w-7xl mx-auto">
          ...
          <div className="flex items-center gap-3">
            <div className="relative">
              ...
              <input 
                type="text" 
                placeholder="Buscar..." 
                className="pl-10 pr-4 py-2 bg-slate-900 border border-slate-800 rounded-lg text-sm focus:outline-none focus:border-slate-500 focus:ring-1 focus:ring-slate-500 transition-all"
              />
            </div>
            ...
            <button className="flex items-center gap-2 px-4 py-2 bg-slate-600 hover:bg-slate-500 text-white rounded-lg transition-colors font-medium text-sm">
              <Plus size={18} />
              Novo Registro
            </button>
          </div>
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden">
            ...
            {[1, 2, 3, 4, 5].map((i) => (
              <div key={i} className="p-6 hover:bg-slate-800/20 transition-colors flex items-center justify-between group cursor-pointer">
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 rounded-full bg-slate-800 flex items-center justify-center text-slate-400 group-hover:text-slate-400 group-hover:bg-slate-500/10 transition-colors">
                    <Settings size={20} />
                  </div>
                  <div>
          ...
          <div className="p-4 border-t border-slate-800 flex justify-center">
            <button className="text-sm font-medium text-slate-400 hover:text-slate-300 flex items-center gap-1 transition-colors">
