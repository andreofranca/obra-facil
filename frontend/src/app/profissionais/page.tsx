...
type Profissional = {
  ...
  ativo: boolean;
  avaliacaoMedia: number | null; // Assumindo que a API pode retornar ou eu moco visualmente
  user: {
    name: string;
  };
  servicos: {
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {profissionais.map((profissional, idx) => {
              ...
              return (
                ...
                >
                  <Card
                    className="flex flex-col gap-5 min-h-[340px] p-6 cursor-pointer transition-all duration-300 ease-out bg-neutral-surface border border-neutral-border rounded-2xl shadow-soft group-hover:shadow-elevated group-hover:border-brand-primary group-focus-visible:shadow-elevated group-focus-visible:border-brand-primary h-full relative overflow-hidden"
                  >
                    {/* Badge lateral (opcional premium) */}
                    ...
                    <div className="flex items-start gap-4">
                      <div
                        aria-hidden="true"
                        className="flex flex-none items-center justify-center w-14 h-14 bg-gradient-to-br from-brand-primary to-brand-secondary rounded-full text-neutral-white font-sans text-lg font-bold shadow-soft"
                      >
                        {initials}
                      </div>
                      <div className="flex-1 overflow-hidden pt-1">
                        <h2 className="text-neutral-text font-sans text-xl font-bold leading-tight truncate">
                    ...
                    <div className="flex flex-wrap items-center gap-3">
                      <div className="flex items-center gap-1.5 bg-neutral-background px-2.5 py-1 rounded-md">
                        <Star className="w-3.5 h-3.5 text-[#F59E0B] fill-[#F59E0B]" />
                        <span className="text-neutral-text text-sm font-bold">{rating}</span>
                        <span className="text-neutral-muted text-xs">({totalReviews})</span>
                      </div>
                      
                      <div className="flex items-center gap-1.5 bg-neutral-background px-2.5 py-1 rounded-md">
                        <MapPin className="w-3.5 h-3.5 text-neutral-muted" />
                        <span className="text-neutral-text text-xs font-medium">São Paulo, SP</span> {/* Placeholder visual exigido */}
                      </div>
                    </div>

                    <p className="text-neutral-muted font-sans text-sm leading-relaxed flex-1 line-clamp-3">
                    ...
                    <div className="mt-auto pt-4 flex items-center justify-between border-t border-neutral-border/50">
                      <Badge tone={profissional.ativo ? "success" : "neutral"} className="rounded-md">
                        {profissional.ativo ? "Disponível" : "Ocupado"}
                      </Badge>
                      
                      <span className="text-brand-primary text-sm font-bold group-hover:translate-x-1 transition-transform flex items-center gap-1">
                        Ver perfil
                      </span>
                    </div>
                  </Card>
